			...
			ros_sub_imu: {
				...
				events: {
                    viewAdd: [
                        function() {
                            if (Entry.container) {
								Entry.container.inputValue.setName("Imu");
				...
				func: function(sprite, script) {
					let inputModel = Entry.container.inputValue;
                    let inputView = Entry.stage.inputField;
					Entry.container.setInputValue(imu);
					var imu_s = new ROSLIB.Topic({
				        ros : ros,
						name : "/imu",
						messageType : "sensor_msgs/Imu"
					});
					imu_s.subscribe(function(message) {
						imu = message.data
						console.log('Received message on ' + imu_s.name + ': ' + message.data);
					});
					...
					us_s.subscribe(function(message) {
						if (cnt==1){
							if (Entry.container) {
								...
								Entry.variableContainer.setListName_us("초음파")//이름 바꿔주기
								cnt = cnt+1;
                            }
						}
